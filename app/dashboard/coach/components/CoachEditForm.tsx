...
import { Coach, LocationData } from '@/app/coach/types/coach';
import TambonSearch from '@/app/coach/common/TambonSearch';
...
export default function CoachEditForm({ coach, isAdmin }: CoachEditFormProps) {
  ...
  const registeredBatchIds = Array.isArray(coach?.batchParticipations) 
    ? coach.batchParticipations.map(p => p.batchId)
    : [];

  // สร้าง initialLocation สำหรับ TambonSearch
  const initialLocation: LocationData | undefined = coach?.location
    ? {
  ...
  const [formData, setFormData] = useState<FormDataState>({
    ...
    coachExperience: coach?.coachExperience?.toString() || "0",
    participationCount: coach?.participationCount?.toString() || "0",
  ...
  // สถานะเพื่อเก็บข้อมูลที่เลือกจาก TambonSearch
    ...
    const fetchBatches = async () => {
      try {
        ...
        if (response && response.success) {
          setAvailableBatches(
            (response.data as TrainingBatchWithCount[]) || []
          );
        }
      } catch (error) {
  ...
  // ฟังก์ชันสำหรับจัดการการเลือกตำบล
  const handleSelectLocation = (location: LocationData) => {
    // อัพเดทข้อมูลใน state เพื่อใช้ในการส่งฟอร์ม
    handleInputChange("district", location.district || "");
    handleInputChange("amphoe", location.amphoe || "");
    handleInputChange("province", location.province || "");
    handleInputChange("zone", location.zone || "");

    // อัพเดท selectedLocation เพื่อแสดงผลในหน้าจอ
  ...
  const formatDateRange = (start: string | Date, end: string | Date) => {
    ...
    if (
      ...
      return `วันที่ ${startDate.getDate()}-${endDate.getDate()} ${startDate.toLocaleDateString(
        thaiLocale,
      ...
      return `${startDate.toLocaleDateString(
        thaiLocale,
        options
      )} - ${endDate.toLocaleDateString(thaiLocale, options)}`;
    }
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      Object.entries(formData).forEach(([key, value]) => {
        if (key === "selectedBatchIds" && Array.isArray(value)) {
          ...
          if (value.length > 0) {
            ...
            submitData.append("selectedBatchIds[]", "");
          }
        } else {
      ...
      // ตรวจสอบว่า result ไม่เป็น undefined ก่อนเข้าถึง property success
      if (result && result.success) {
        ...
        // ถ้า result เป็น undefined หรือไม่มี success
    ...
    <form onSubmit={handleSubmit}>
      ...
      <Tabs defaultValue="personal">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div className="space-y-2">
                  <Label htmlFor="idCardNumber">
                    เลขบัตรประชาชน
                    {!isAdmin && <span className="text-xs text-orange-500 ml-1">(เฉพาะแอดมินเท่านั้นที่สามารถแก้ไข)</span>}
                  </Label>
                  <Input
                    id="idCardNumber"
        ...
        <TabsContent value="location">
          <Card>
            <CardContent className="pt-6 space-y-4">
              {/* เพิ่ม TambonSearch component */}
              <div className="mb-6">
                <Label htmlFor="tambonSearch" className="block mb-2">
                ...
                <p className="mt-2 text-sm text-gray-600">
                  พิมพ์ชื่อตำบล, อำเภอ หรือจังหวัดที่ต้องการ
                  และเลือกจากรายการที่แสดง ระบบจะกรอกข้อมูลให้อัตโนมัติ
                </p>
              </div>

              {/* แสดงข้อมูลที่เลือกจาก TambonSearch */}
              <div className="mt-4 bg-gray-50 p-4 rounded-lg">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div>
                    <p className="text-sm font-medium text-gray-500">ตำบล</p>
                    <p className="font-medium">{formData.district || "-"}</p>
                  </div>
                  <div>
                    <p className="text-sm font-medium text-gray-500">อำเภอ</p>
                    <p className="font-medium">{formData.amphoe || "-"}</p>
                  </div>
                  <div>
                  ...
                  <div>
                    <p className="text-sm font-medium text-gray-500">ภูมิภาค</p>
                    <p className="font-medium">{formData.zone || "-"}</p>
                  </div>
                </div>
              </div>

              {/* ใช้ input ที่มองไม่เห็นเพื่อเก็บค่า */}
              <input type="hidden" name="district" value={formData.district} />
              <input type="hidden" name="amphoe" value={formData.amphoe} />
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div className="space-y-2">
                  ...
                  <Select
                    ...
                    onValueChange={(value) =>
                      handleInputChange("coachStatus", value)
                    }
                  >
                    ...
                    <SelectContent>
                      ...
                      <SelectItem value="GOVERNMENT_EMPLOYEE">
                        ลูกจ้างราชการ
                      </SelectItem>
                      <SelectItem value="ACADEMY_EMPLOYEE">
                        ลูกจ้าง Academy
                      </SelectItem>
                      <SelectItem value="ACADEMY_OWNER">
                        เจ้าของ Academy
                      </SelectItem>
                ...
                <div className="space-y-2">
                  ...
                  <Input
                    ...
                    onChange={(e) =>
                      handleInputChange("coachExperience", e.target.value)
                    }
                  />
                </div>
                ...
                <div className="space-y-2">
                  ...
                  <Input
                    ...
                    onChange={(e) =>
                      handleInputChange("participationCount", e.target.value)
                    }
                  />
                </div>
              </div>
              ...
              {loadingBatches ? (
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  {availableBatches.map((batch) => {
                    const isSelected = formData.selectedBatchIds.includes(
                      batch.id
                    );
                    ...
                    const seatsRemaining = Math.max(
                      0,
                      batch.maxParticipants - participantsCount
                    );
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div className="space-y-2">
                  ...
                  <Select
                    ...
                    onValueChange={(value) =>
                      handleInputChange("shirtSize", value)
                    }
                  >
