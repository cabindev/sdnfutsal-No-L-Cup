...
export default async function BatchDetailPage({ 
  ...
  return (
    <div className="p-6 max-w-7xl mx-auto">
      <div className="flex items-center justify-between mb-6">
        ...
        <div className="flex space-x-2">
          <Link href={`/dashboard/training/batch/edit/${batch.id}`}>
            <Button className="flex items-center gap-2 bg-futsal-blue hover:bg-futsal-blue/90">
              <Edit className="h-4 w-4" />
              แก้ไขรุ่นอบรม
            </Button>
          </Link>
          
          <BatchDeleteButton batchId={batch.id} batchName={`${batch.batchNumber}/${batch.year}`} />
        </div>
      </div>
      
      <div className="mb-6">
        <h1 className="text-2xl font-bold text-futsal-navy">รุ่นที่ {batch.batchNumber}/{batch.year}</h1>
          ...
          <CardContent className="space-y-4">
            <div className="flex items-start">
              ...
              <div>
                ...
                <p className="text-gray-600">{formatDate(batch.startDate)} - {formatDate(batch.endDate)}</p>
              </div>
            </div>
            ...
            <div className="flex items-start">
              ...
              <div>
                ...
                <p className="text-gray-600">{batch._count.participants} / {batch.maxParticipants} คน</p>
              </div>
            </div>
            ...
            <div className="flex items-start">
              ...
              <div>
                ...
                <p className="text-gray-600">{formatDate(batch.registrationEndDate)}</p>
              </div>
            </div>
            ...
            {batch.description && (
              <div className="pt-4 border-t">
                ...
                <p className="text-gray-600 whitespace-pre-line">{batch.description}</p>
              </div>
            )}
          </CardContent>
        </Card>
        ...
        <Card>
          ...
          <CardContent>
            <div className="space-y-4">
              ...
              {Object.entries(participantsByStatus).map(([status, participants]) => (
                <div key={status} className="flex items-center justify-between">
                  <span className="text-gray-700">{statusLabels[status] || status}</span>
                  <span className="font-medium">{participants.length} คน</span>
                </div>
              ))}
              
              <div className="flex items-center justify-between pt-3 border-t">
                <span className="font-medium">ทั้งหมด</span>
                <span className="font-medium">{batch._count.participants} คน</span>
              </div>
            </div>
          </CardContent>
        </Card>
      ...
      <div className="mb-6">
        <h2 className="text-xl font-bold text-futsal-navy mb-4">รายชื่อผู้เข้าร่วม</h2>
        
        {batch.participants.length === 0 ? (
          <div className="bg-gray-50 rounded-lg p-8 text-center">
            <Users className="h-12 w-12 mx-auto text-gray-400 mb-4" />
            <h3 className="text-lg font-medium text-gray-800 mb-1">ยังไม่มีผู้เข้าร่วม</h3>
            <p className="text-gray-600">ยังไม่มีโค้ชลงทะเบียนเข้าร่วมรุ่นนี้</p>
          </div>
        ) : (
              ...
              <table className="w-full">
                <thead>
                  <tr className="bg-gray-50 border-b">
                    ...
                    <th className="text-left py-3 px-4 font-medium text-gray-700">พื้นที่</th>
                    <th className="text-left py-3 px-4 font-medium text-gray-700">วันที่ลงทะเบียน</th>
                  ...
                  {batch.participants.map((participant) => (
                    <tr key={participant.id} className="hover:bg-gray-50">
                      <td className="py-3 px-4">
                        <Link href={`/dashboard/coach/${participant.coach.id}`} className="text-futsal-blue hover:underline">
                          {participant.coach.user.firstName} {participant.coach.user.lastName}
                          {participant.coach.nickname && <span> "{participant.coach.nickname}"</span>}
                        </Link>
                      </td>
                      <td className="py-3 px-4">{participant.coach.user.email}</td>
                      <td className="py-3 px-4">
                        {participant.coach.location ? 
                          `${participant.coach.location.amphoe}, ${participant.coach.location.province}` : 
                          '-'}
                      </td>
                      <td className="py-3 px-4">{formatDate(participant.registeredAt)}</td>
                      <td className="py-3 px-4">
